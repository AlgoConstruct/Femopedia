...
- A public, server-rendered bilingual blog sharing one content spine with the knowledge base, as the acquisition channel.
...
| Content management | Wagtail, headless, inside the same Django project | Editing, bilingual pairs, clinician review workflow |
...
- `cms` — Wagtail page types, review workflow, publish hooks, public API
- `knowledge` — embeddings, retrieval, graph queries
...
(content lives in Wagtail; see section 6)
ContentEntry    Wagtail page types KnowledgeEntry and BlogPost, sharing a
                base with slug, title, body, lang, tier, reviewer,
                reviewed_at, sources, topic_ids, locale link
...
## 6. Content platform and SEO

### One content spine

The knowledge base and the blog are the same system. Two Wagtail page types, `KnowledgeEntry` and `BlogPost`, share a base model carrying `lang`, `tier`, `reviewer`, `reviewed_at`, `sources`, and `topic_ids`. A published post is simultaneously a page Google can rank and a chunk the companion retrieves from. Running two content systems would mean writing everything twice and drifting between what search engines see and what the AI says.

### Why Wagtail rather than Directus or a custom build

Directus is a second runtime, a second container, and a second upgrade path, which is the wrong trade for a solo operator already running four datastores. Markdown files in the repository are quick to build but make every content change a git commit, which locks out the clinician reviewer and any future content hire — the wrong constraint for a product whose moat is reviewed content.

Wagtail supplies three things this spec would otherwise have to build:

1. **Moderation workflow.** Draft, review, approve, publish, with revisions and diffs. Section 5 requires a named clinician to sign off on tier-1 content; this is that mechanism.
2. **`wagtail-localize`.** Nepali and English exist as linked translations of one page rather than two unrelated records.
3. **Same Django project, same Postgres, one deploy, one authentication system.** No new service to operate.

### Publish pipeline

Publishing a page fires a Wagtail signal that queues a Celery job: re-chunk, re-embed into the language-appropriate Chroma collection, and update the Neo4j `EXPLAINS` and `CONTRADICTED_BY` edges. Unpublishing removes the chunks. Retrieval therefore never serves content that is unpublished or awaiting review.

### Rendering

Wagtail runs headless. The TanStack application serves `/blog/*` through server-rendered routes that read the Wagtail API v2. This keeps the blog inside the existing design system, since the blog is the front door and should not look like a different product.

The alternative considered and rejected for v1: routing `/blog/*` at nginx directly to Wagtail templates. Cheaper, and good for SEO, but it leaves two visual systems to keep in sync.

### Search and answer-engine optimisation

- Server-rendered pages at real, stable URLs
- `Article` and `FAQPage` schema.org markup
- `hreflang` across the Nepali and English pairs
- Sitemap generated by Wagtail
- Every post structured as an answer to a question a woman actually types, with the answer stated plainly in the first paragraph

That last point serves both channels at once: it is what answer engines cite, and it is the same shape the companion's answers take.

### Justification for including this in v1

Risk 5 below records distribution as unsolved and as the most likely cause of failure. This is the response to it, and section 13.6 of the strategy document already identifies content-led acquisition as the primary channel.

---

## 7. Routing and cost control
...
**P1.5 — Content platform (week 2).** Wagtail in the Django project, the two page types and their shared base, `wagtail-localize`, the review workflow with the clinician as an approver, the publish-to-Celery hook, the API v2 surface, and the `/blog/*` SSR routes in TanStack with schema.org markup, `hreflang`, and sitemap. Done when a post authored in Wagtail is publicly readable, indexed, and retrievable by the companion.
...
5. **Distribution.** How a woman in Nepal finds this remains the most likely cause of the project failing. P1.5 provides the channel — bilingual, search-visible content sharing the companion's content spine — but a channel is not yet an audience, and organic ranking is slow. Partially addressed, not solved.
